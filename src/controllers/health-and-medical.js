 ...
 * @summary Controller to handle http request for HealthAndMedical model related functions
 * @name HealthAndMedicalController
 * @extends BaseController
 */
module.exports.name = 'HealthAndMedicalController'
...
module.exports.factory = class extends BaseController {
  ...
  constructor(repo, applicantRepo, helper, logger, response, mongoose) {
    ...
    this.on('insert', async (req, doc) => {
      try {
        await this.applicantRepo.update(doc.applicant, { stage: 10 })
      } catch (error) {
        this.log(error)
      }
    })
