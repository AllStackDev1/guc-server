 ...
 * @summary Controller to handle http request for GuardianContactInformation model related functions
 * @name GuardianContactInformationController
 * @extends BaseController
 */
module.exports.name = 'GuardianContactInformationController'
...
module.exports.factory = class extends BaseController {
  ...
  constructor(repo, applicantRepo, helper, logger, response, mongoose) {
    ...
    this.name = 'Guardian contact information'
    this.listening = true
    ...
    this.on('insert', async (req, doc) => {
      try {
        await this.applicantRepo.update(doc.applicant, { stage: 11 })
      } catch (error) {
        this.log(error)
      }
    })
